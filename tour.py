        ...
        while self.app.tour_running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    if event.key == keyboard.KEY_BINDINGS['left'] and self.step >= self.move_step:
                        ...
                        if self.step == self.move_step and self.first_move:
                            self.current_widget.finished = True
                    elif event.key == keyboard.KEY_BINDINGS['right'] and self.step >= self.move_step:
                        self.app.main_player.move[1] = True
                        self.app.main_player.move[0] = False
                        self.app.main_player.flip = False
                        if self.step == self.move_step and self.first_move:
                            self.current_widget.finished = True
                    elif event.key == keyboard.KEY_BINDINGS['jump'] and self.step >= self.jump_step:
                    ...
                    elif event.key == keyboard.KEY_BINDINGS['attack'] and self.step >= self.attack_step:
